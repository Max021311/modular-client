...
import type { CreateCareer, Career } from '~/types/api/career.d'
...
export const useAddCareer = () => {
  ...
  const mutate = async (payload: CreateCareer, token: string): Promise<void> => {
    try {
      pending.value = true
      error.value = null
      data.value = null

      const response = await axios.post<Career>('/careers', payload, {
        headers: {
